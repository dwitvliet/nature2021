function [Best_Models,Scores,Models, allLogEvidence] = wsbmLooper_2(E,ModelInputs,scorefuncs,numTrialPttrn,priorWeightPttrn,idx, outputDir)
...
allLogEvidence = nan(num_models, numel(numTrialPttrn));
if nargout > 2,
        ...
        if isempty(numTrialPttrn)
           ...
           for jdx=1:numel(numTrialPttrn)
                ...
                [~,Model] = wsbm(E, ...
                    ...
                    'mu_0', mu_prior ) ;
               allLogEvidence(mm, jdx) = Model.Para.LogEvidence;
...
save(sprintf('%s/logEvidence%d', outputDir, idx), 'allLogEvidence');
if num_scores == 1, 
